...
class FactionMain extends PluginBase implements Listener {
    ...
    public static $prefix = "§f- §3Empholia §f- ";

    public function onEnable() {

        @mkdir($this->getDataFolder());

        Entity::registerEntity(CoeurDeFaction::class, false, ["EnderCrystal"]);
        ...
        $this->getScheduler()->scheduleRepeatingTask(new FactionClock($this), 30);
        ...
        $this->db->exec("CREATE TABLE IF NOT EXISTS cdf(ID INT PRIMARY KEY, level INT, x INT, y INT, z INT, world TEXT);");
     ...
     * @return array
     */
    public function getFactionMembers($faction) : array {
        $leader = $this->db->query("SELECT player FROM master WHERE faction='$faction';");
        $leaderArray = $leader->fetchArray(SQLITE3_ASSOC);
        $players = array();
        foreach($leaderArray as $key)
        {
    ...
    public function formatMessage($string, $confirm = false) {
        if ($confirm) {
            return self::$prefix . TextFormat::GREEN . "$string";
        } else {
            return self::$prefix . TextFormat::YELLOW . "$string";
        }
    }
    ...
    public function addCdf($faction, $x, $y, $z, $level)
    {
        $stmt = $this->db->prepare("INSERT OR REPLACE INTO cdf(faction, x, y, z, level, world) VALUES (:faction, :x, :y, :z, 1, :level);");
        $stmt->bindValue(":faction", $faction);
        $stmt->bindValue(":x", $x);
        $stmt->bindValue(":y", $y);
        $stmt->bindValue(":z", $z);
     ...
     * @return bool
     */
    public function haveCdf($faction) : bool {
        $result = $this->db->query("SELECT ID FROM cdf WHERE (faction = '$faction');");
        $resultArr = $result->fetchArray(SQLITE3_ASSOC);
        if (empty($resultArr) == false) {
            return true;
        }
